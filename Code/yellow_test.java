public class Solution {
	...
	public Solution() {
		...
		/**
		 * Currently found targets and corresponding threat level. Lower number
		 * means lower threat.
		 */
	...
	long rand() {
		long r = a * previous + c;
		// Note: typically, one chooses only a couple of bits of this value, see
		// link
		previous = r;
		return r;
	}
	 ...
	 * 
	 * TODO this function assumes that the move is always succesfuel. this may
	 * not be the case if we try to move into a wall or an enemy.
	 */
	...
	public void update() {
		if (0 == time) {
		    ...
		    delta = 1;
		    time++;
			currentFuel = API.currentFuel();
		} else {
            previousFuel = currentFuel;
            currentFuel = API.currentFuel();
			delta = previousFuel - currentFuel;
		...
		/*
		 * if (API.identifyTarget()) { //System.out.println("Enemy!"); danger
		 * +=1.8; } else { //System.out.println("No Enemy detected"); if(danger
		 * > 0) danger += -1; } if(danger > 0) API.fireCannon();
		 */
